...
class HelpManager():
    ...
    def filter(self):
        ...
        from .client.tfilter import (BOOLEAN_FILTERS, COMPARATIVE_FILTERS)
        lines = [
            'FILTERING TORRENTS',
        ...
        for fname,f in sorted(BOOLEAN_FILTERS.items()):
            lines.append('\t\t{} \t{}'.format(', '.join((fname,)+f.aliases), f.description))

        lines += ['', '\tCOMPARATIVE FILTERS']
        for fname,f in sorted(COMPARATIVE_FILTERS.items()):
            lines.append('\t\t{} \t{}'.format(', '.join((fname,)+f.aliases), f.description))
        return finalize_lines(lines)
