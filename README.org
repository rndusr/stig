...
** Configuration and Scripting
   ...
   ~$HOME/.config/stig/rc~ if you haven't changed XDG_CONFIG_HOME.  Each line is
   a command that is called during startup.  Commands given as CLI arguments are
   called after the rc commands.

   Run ~stig help rcfile~ for more information.
