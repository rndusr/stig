* stig
  ...
  stig is still in alpha status to indicate that behaviour may change
  (i.e. "breaking userspace") with a new release. It does *not* mean you should
  expect bugs.
...
*** Extras
   ...
   #+BEGIN_SRC sh
   $ pip3 install 'stig[create,geoip]'
   #+END_SRC
