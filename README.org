...
*** Pipsi (recommended)
    ...
    ~\~/.local/venvs/~ that include all dependencies.  Executables are
    sym-linked to ~\~/.local/bin/~.
...
*** Pip
    ...
    (~/usr/local~) or user-wide (~\~/.local~) environment.

    #+BEGIN_SRC sh
    $ pip3 install stig         # Installs in /usr/local/
    $ pip3 install --user stig  # Installs in ~/.local/
    #+END_SRC
...
*** Extras
   The following extras are available to enable optional features:
     ...
     - ~setproctitle~ :: Change process title when running in tmux session (this
                         requires Python headers; e.g. ~apt-get install
                         libpython3-dev~)
...
*** Developers
    If you have cloned the Git repository and want to make your changes
    effective immediately, you can either run ~python3 -m stig~ in the project
    directory or install stig with ~pip3~ from a local path and pass the
    ~--editable~ option.
