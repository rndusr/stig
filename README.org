...
** Features
   ...
   - Everything is done via *commands*, and they are invoked
     - through *chained[fn:1] keybindings*,
...
[fn:1] Bind multiple keypresses to an action; for example, ~f~ followed by ~a~
lists active torrents while ~f p~ lists paused torrents.
